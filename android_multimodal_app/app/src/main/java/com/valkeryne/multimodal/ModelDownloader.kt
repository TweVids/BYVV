...
object ModelDownloader {
    ...
    val REQUIRED_MODELS = listOf(
        ...
        ModelFileInfo("tts/vi_VN-vivos-x_low.onnx.json", "vi_VN-vivos-x_low.onnx.json", 5592L, false),
    ...
    fun isModelValid(file: File, info: ModelFileInfo): Boolean {
        ...
        // Size must match expected byte length exactly
    ...
    suspend fun downloadAllModels(
        ...
        try {
            ...
            for ((idx, info) in REQUIRED_MODELS.withIndex()) {
                ...
                // If this file is already fully and correctly downloaded, advance immediately
                if (isModelValid(localFile, info)) {
                    onProgress(info.fileName, 100, "[${idx + 1}/${REQUIRED_MODELS.size}] ${info.fileName} Ready")
                    continue
                }
